...
当前仍不是生产上线版本。原因是生产配置、真实组织数据、CI/E2E、网关日志脱敏、密钥治理、HTTPS、备份、监控告警和完整部署运维仍未闭环。
...
| 安全治理 | URL token 已清零，但生产还需访问日志 query 脱敏、密钥管理、HTTPS、CSP、审计策略 | 生产上线前必须补齐 |
| CI/E2E | 构建与脚本仍为本地执行，未接入 CI 自动跑 | 交付质量门禁不足 |
| 生产部署 | Nginx、HTTPS、备份、监控告警、数据库迁移流程未完成 | 不能直接生产上线 |
| 工作区提交 | 当前仍是大量未跟踪文件，需要拆分提交/PR | 评审成本较高 |
...
1. 按上面的拆分方案提交当前工作区，不要 squash 成一个巨大提交。
2. 将 `mvn clean package -DskipTests`、`npm run build:prod`、`daily-lkj-mvp-check.ps1`、`security-gap-scan.ps1` 接入 CI。
